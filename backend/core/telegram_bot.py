...
async def _dispatch_command(chat_id: str, text: str):
    ...
    if cmd in ("hixiit", "hixit", "higgsfield"):
        ...
        for src in st.get("key_sources") or []:
            ...
            # Форма значения важнее факта «задан»: неверную длину человек иначе
            # не увидит вообще — в панели хостинга значение скрыто точками.
            shape = ""
        ...
        # Чужой бесплатный генератор рисует не то и ставит свой водяной знак.
        # Человек должен видеть, включена ли такая подмена, — иначе непонятно,
        # откуда взялся кадр, которого он не просил.
        from core.hixiit import free_draft_allowed
        lines.append("🎭 Черновик бесплатным генератором: "
                     + ("включён — кадр может быть не по запросу"
                        if free_draft_allowed() else "выключен"))
        ...
        # Живой статус может быть зелёным, а генерация всё равно падать: проверка
        # доступа и сама генерация — разные запросы. Для картинок цепочка к тому
        # же подменяет результат бесплатным генератором и возвращает «получилось»,
        # поэтому без этого блока отказ Higgsfield не виден вообще.
        from core.hixiit import recent_failures
        fails = await recent_failures()
        if fails:
            lines += ["", "⚠️ <b>Последние неудачные генерации</b>"]
            for f in fails:
                lines.append(f"   {f['when']} · {f['what']}: {f['error']}")
    ...
    if cmd == "diag":
        ...
        # Ключ может быть задан дважды — в дашборде и в переменных хостинга.
        # Побеждает дашборд, и новый ключ, положенный в Render поверх старого
        # сохранённого, не даст никакого эффекта. Молчать об этом нельзя:
        # человек будет искать поломку там, где её нет.
        from core.credentials import load_into_env
        shadowed = (await load_into_env()).get("shadowed") or []
        if shadowed:
            lines += [
                "",
                "ℹ️ Заданы и в дашборде, и в переменных хостинга:",
                "   " + ", ".join(shadowed),
                "Используется значение из дашборда. Чтобы работало из хостинга — "
                "удалите ключ в Подключениях.",
            ]

        # Ключ, названный не так, как его читает код, для системы всё равно что
        # отсутствует — а в списке переменных хостинга он выглядит заданным.
        # Это тот случай, когда чинят код вместо имени.
        from core import env_audit
        lines += env_audit.as_lines(env_audit.misnamed())
...
async def _show_model_menu(chat_id: str):
    ...
    lines += ["", "AUTO — система подбирает модель под задачу.",
              "Показаны только модели, которые реально можно запустить."]

    # Короткий список — не поломка, а следствие пути исполнения. Без этой
    # строки человек видит одну модель картинок и не понимает, куда делись
    # остальные, которые есть у него в кабинете.
    from core.hixiit import mcp_configured
    if not mcp_configured():
        lines += ["", "ℹ️ Сейчас работает путь по ключу и секрету — у него одна "
                      "картиночная модель (Soul). Весь каталог аккаунта "
                      "(nano_banana, seedream, kling и другие) открывается при "
                      "подключённом MCP: переменная <code>HIGGSFIELD_MCP_URL</code>."]
