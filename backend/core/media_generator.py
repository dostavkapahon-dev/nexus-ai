...
_PROMPT_SYSTEM = (
    "Ты — художник-постановщик. Запрос может быть на любом языке — переведи его "
    "на английский и разверни в короткое описание КАДРА для генератора "
    "изображений: что в кадре, ракурс, свет, стиль, фон. Сохрани смысл запроса, "
    "ничего не выдумывая сверх него. Ответ — ТОЛЬКО английское описание одной "
    "строкой, без вступлений и пояснений."
)
...
def _needs_enrichment(prompt: str) -> bool:
    """Нужно ли переписать запрос перед генерацией.

    Модели изображений обучены на английских описаниях, поэтому русский текст
    переписывается ВСЕГДА, какой бы длины он ни был. Раньше здесь стояла отсечка
    по длине, и развёрнутая фраза по-русски уходила в генератор как есть —
    человек описывал кадр подробно и получал результат хуже, чем от одного слова.

    Английский запрос трогаем, только если он совсем короткий: дописывать за
    человека подробное описание, которое он уже составил, — значит менять его
    замысел.
    """
...
async def enrich_image_prompt(prompt: str) -> str:
    ...
    if not better or not (3 <= len(better.split()) <= 80):
        return prompt
    # И иногда отвечает по-русски, хотя просили английское описание. Такой
    # «перевод» бесполезен — генератор снова получит кириллицу.
    if has_cyrillic(prompt) and has_cyrillic(better):
        return prompt
