...
# Модели берутся из ЖИВОГО каталога аккаунта, а не из памяти: soul_2 и soul_v2
# из него исчезли, и запрос с ними возвращал «400: Unavailable model». Порядок —
# по пригодности для соцсетей: фотореализм, затем быстрая, затем фоновая.
# soul_cast сюда не входит: он умеет только 16:9 и требует обученного персонажа.
#
# У REST нет способа спросить каталог, поэтому список зашит и однажды снова
# устареет. Чтобы это не стоило круга переписки, HIGGSFIELD_IMAGE_MODEL
# перекрывает перебор, а отказ «Unavailable model» прямо называет причину.
IMAGE_MODELS_REST = ("recraft_v4_1", "z_image", "soul_location")
