...
- 🔴 Найден дефект по жалобе «три раза сменил ключ, ошибка та же»: проверка ключа
  Instagram на странице «Ключи» всегда шла на `graph.facebook.com`, поэтому
  исправный токен `IGAA…` (Instagram Login) получал от Facebook «Cannot parse
  access token». Адрес теперь выбирается по виду токена. Проверено «с другой
  стороны»: на прежнем коде новый тест падает ровно с этой ошибкой.
