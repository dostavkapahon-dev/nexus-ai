...
Прогон: `cd backend && python -m pytest -q` → **373 passed** (после вебхуков и прямого подключения Instagram;
...
| Вебхуки Instagram (подпись, токен) | 03 | `tests/test_webhooks.py` | 29 | 🟢 PASS |
| Подключение Instagram по токену | 03 | `tests/test_ig_connect.py` | 37 | 🟢 PASS |
...
- Сохранение ключа вживую: записан через `/api/connections`, процесс перезапущен,
  ключ и тип токена на месте, Instagram считается настроенным → 🟢 PASS.
- Настройки вебхука вживую: эндпоинт отдаёт готовый адрес и генерирует токен
  подтверждения; этим же токеном подписка подтверждается (challenge вернулся,
  200), чужой — 403; повторный заход значение не перевыпускает → 🟢 PASS.
