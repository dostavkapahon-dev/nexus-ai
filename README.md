...
### Лёгкое подключение любого ИИ

- **OpenRouter** — один ключ открывает **сотни моделей** (Llama, Qwen, Grok,
  DeepSeek и др.). В промпте агента укажите модель как `vendor/model`
  (например `meta-llama/llama-3.1-70b-instruct`) — запрос уйдёт через OpenRouter.
- **Grok (xAI)**, **Mistral** — отдельные ключи, OpenAI-совместимые.
- **Свой ИИ / локальная LLM** — задайте `CUSTOM_AI_BASE_URL`
  (Ollama `http://localhost:11434/v1`, vLLM, LM Studio, корпоративный шлюз).

Добавить новый провайдер в код — одна строка в `OPENAI_COMPATIBLE`
(`backend/core/ai_router.py`). Gemini работает через лёгкий REST-клиент
(`core/gemini_rest.py`) без тяжёлых SDK — приложение стабильно поднимается
в любой среде.
