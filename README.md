...
Картинку делает **HiggsField**, и модель подбирается под кадр — одна модель не
хороша во всём:

| Что в кадре | Модель |
|---|---|
| текст, надпись, инфографика, макет | `nano_banana_2` (4K, чёткий текст) |
| товар, реклама, упаковка | `marketing_studio_image` |
| киношный лук | `cinematic_studio_2_5` |
| обученный персонаж (`soul_id`) | `soul_2` |
| черновик на проверку композиции | `nano_banana_flash` (дёшево) |

Без `HIGGSFIELD_API_KEY` работает бесплатный запасной путь (Pollinations), но он
умеет только рисовать заново.

**Правка картинки.** На согласовании нажмите «✏️ Правки» и напишите замечание.
Если оно про визуал («поменяй фон», «другой ракурс»), кадр перерисовывается
**image-to-image от исходника** — тот же человек в той же одежде, меняется
названное. Замечания по тексту кадр не трогают: перерисовывать картинку из-за
правки подписи — жечь деньги впустую.
