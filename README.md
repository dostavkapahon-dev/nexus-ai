...
## Чтобы ключи не пропадали при деплое

На Render диск контейнера эфемерный: каждый деплой стирает базу вместе с
доступами. Переменные окружения при этом сохраняются — этим и пользуемся.

Правильный путь (пять минут, навсегда): завести бесплатный Postgres
(supabase.com) и задать `DATABASE_URL` в Render → Environment.

Если пока не хочется — копия ключей в переменной:

1. Подключения → API/Credentials → **«Скачать копию»** (задайте пароль от 8 символов);
2. содержимое файла вставьте в Render → Environment как `NEXUS_KEYS_BACKUP`;
3. пароль — туда же, как `NEXUS_KEYS_PASSWORD`.

При каждом старте система развернёт ключи сама. В переменной они лежат
зашифрованными, пароль отдельно.

## Клод-исполнитель на вашей подписке (без оплаты API)

Подписку claude.ai нельзя подключить к серверу как API. Но можно наоборот:
Claude Code под вашим аккаунтом сам забирает задания из системы.

```bash
export NEXUS_URL=https://ваш-сервис.onrender.com
export NEXUS_PASSWORD=<пароль администратора>
python tools/claude_worker.py
```

Пока воркер запущен, система складывает вопросы в очередь и получает ответы за
секунды. Остановили — система возвращается в автономный режим: без моделей
собирает заготовку по шаблону и никого не дёргает.

## Настройка целиком из Telegram

Веб нужен не всегда: систему можно поднять с телефона.

| Команда | Что делает |
|---|---|
| `/setup` | чеклист по способностям: что уже работает, что включится, если подключить ещё одно, и где это взять |
| `/key имя значение` | подключить ключ прямо из чата (`/key` без аргументов — список имён) |
| `/autopub` | автопубликация: общий рубильник и режим каждой площадки (`manual` · `confirm` · `auto`) |
| `/diag` | живая проверка ключей ИИ — какие реально отвечают |

`/setup` отвечает не галочками по ключам, а по задачам: «умеет ли система
публиковать сама», «может ли разобрать мой Instagram», «сделает ли видео».
Каждая недостающая строка называет имя ключа, так что её тут же можно исполнить
через `/key`.

Сообщение с ключом бот удаляет из чата сразу после сохранения, а в ленту
дашборда пишет команду без значения. Две переменные так задать нельзя —
`DATABASE_URL` и `NEXUS_SECRET_KEY` читаются при старте процесса, им место
в Render → Environment; `/setup` про это и говорит.
