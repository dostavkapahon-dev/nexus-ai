...
## Реестр возможностей (`core/capabilities.py`)

Отдельный слой честности, не пересекающийся с `/diag` (что настроено) и
`/system_test` (что отвечает прямо сейчас). Реестр отвечает на третий вопрос —
**что уже получалось**, и это единственный вопрос, ответ на который можно
обещать пользователю.

- Записи делают **рабочие пути**: `hixiit._generate_once` (картинка/видео),
  `publishers/telegram_pub`, `publishers/instagram_pub`, `websearch.search`,
  `health._probe_memory` (настоящая запись и чтение). Проверки статуса ничего
  не записывают — иначе зелёное снова означало бы «ключ принят».
- Хранение — KV в таблице `Connection`, ключ `cap:<id>`.
- Доказательство живёт `PROOF_HOURS = 72`; дальше состояние честно откатывается
  в «не проверено».
- `capabilities.can(id)` — строгий ответ для оркестратора, чтобы он не обещал
  неподтверждённое. Команда `/can` — то же самое словами.
- Реестр — наблюдатель: любая его ошибка логируется и никогда не ломает саму
  работу.
