...
лента событий, выбранная стратегия). Дополнительно: Память агента (`skills.json`,
`brand_voice.txt`, `hook_history.json`) больше не теряется: она продублирована в
таблицу `agent_state` и разворачивается на диск при старте (`core/file_state.py`).
