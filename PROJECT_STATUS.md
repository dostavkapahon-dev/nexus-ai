...
## Доставка результата владельцу ✅

Три бага одной цепочки, дававшие «сгенерировал, а в Telegram ничего не прислал»:

| Что было | Где | Стало |
|---|---|---|
| адресат читался только из `TELEGRAM_CHAT_ID`, хотя владельца штатно закрепляет первый `/start` — 21 точка | `scheduler`, `moderation`, `content_factory`, `cost_tracker`, … | единая `notify.owner_chat()`: переменная, иначе закреплённый владелец |
| ответ Telegram не проверялся: `ok:false` («не смог забрать картинку по ссылке») считался успехом | `core/moderation.py::_tg` | ответ читается; при отказе по медиа — запасная отправка текстом с кнопками и причиной |
| бесплатный генератор отчитывался об успехе, ни разу не сходив по своей ссылке | `core/media_generator.py` | ссылка проверяется запросом (две попытки), отказ пишется в журнал |
